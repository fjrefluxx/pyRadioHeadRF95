...
public:
    ...
    /// julian.zobel@gmail.com 12.03.2021
    /// Set the 8bit / 1B sync word for LoRa. Standard in Lora is 0x34, but seems to be initialized as 0x12 on the RF95.
    void setSyncWord(uint8_t syncWord);
	int getSyncWord();

	/// julian.zobel@gmail.com 18.03.2021
    /// explicit header mode setters and getters for use without reliable datagram
    ///
    /// modes
    /// 0: no header
    /// 1: just FROM header
    /// 2: full explicit header
	void setExplicitHeaderMode(uint8_t mode);
	int getExplicitHeaderMode();

	void setImplicitHeaderMode(bool on, uint8_t expectedPayloadLength);
	bool getImplicitHeaderMode();

	/// getters for transmission header information
	int getTXHeaderTo();
	int getTXHeaderFrom();
	int getTXHeaderID();
    ...
    // returns the currently applicable header length
	int getHeaderLength();
...
//#endif
    ...
    uint8_t				RH_RF95_HEADER_LEN;

    uint8_t RH_RF95_MAX_MESSAGE_LEN = RH_RF95_MAX_PAYLOAD_LEN;

    uint8_t RH_RF95_IMPLICIT_HEADER_MODE_EXPECTED_PAYLOAD_LENGTH;
...
private:
    ...
    int32_t              _lastRawRssi;
    
    bool                 _lastCrcOk;
    
    bool                 _checkCrc;

    uint8_t 			 _explicitHeaderMode;
};
